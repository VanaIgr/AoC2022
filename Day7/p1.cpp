...
struct CalcResult {
    ...
    int foldersAnswerSum;
};

static CalcResult calcResult(Folder const &folder) {
    auto folderSize = decltype(CalcResult::folderSize){};
    auto foldersAnswerSum = decltype(CalcResult::foldersAnswerSum){};
    auto subfolders = folder.subfolders;
    while(subfolders) {
        auto const result = calcResult(subfolders->cur);
        folderSize += result.folderSize;
        foldersAnswerSum += result.foldersAnswerSum;
        subfolders = subfolders->next;
    }
    folderSize += folder.filesSize;
    return { folderSize, foldersAnswerSum + (folderSize <= 100'000 ? folderSize : 0) };
}

static int calcFoldersAnswer(Folder &folder) {
    return calcResult(folder).foldersAnswerSum;
}
