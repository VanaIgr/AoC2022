...
int main() {
    ...
    auto const maxWidth = (maxPos.y - sandPos.y)*2 + 1; //floor is 1 lower -> 3,   2 lower -> 5,   3 lower -> 7

    minPos.x = std::min(minPos.x, sandPos.x - maxWidth/2);
    maxPos.x = std::max(maxPos.x, sandPos.x + maxWidth/2);
    ...
    while(true) {
        ...
        while(true) {
            if(!isAir(cx, cy)) goto finish;
            if(!inBounds(cx, cy)) exit(25);
            auto const sx = cx;
            auto const sy = cy;
